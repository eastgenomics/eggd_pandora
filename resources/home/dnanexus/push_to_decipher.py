...
def submit_data_to_decipher(case, submitter_id):
    ...
    response = requests.request(
        "POST", API_URL + PATIENT_URL, data=patient_json, headers=headers
        )
    ...
    if 'errors' in response_json.keys():
        if response_json['errors'][0]['detail'] == (
            ...
            updated_response = requests.request(
                "GET", API_URL + PATIENT_URL, headers=headers
                )
    ...
    for variant_dict in variant_dict_list:
        ...
        response = requests.request(
            "POST", API_URL + VARIANT_URL,
            data=variant_json_to_submit, headers=headers
            )
        print(response.text)
